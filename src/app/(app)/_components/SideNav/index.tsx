        ...
        <div className='rounded-2xl border border-[#FBF4F4] py-6 pb-0 w-full'>
            ...
            <div className='h-full overflow-hidden'>
                <div className={`flex flex-col items-center gap-6 mb-10 pb-6 border-b`}>
                    <Link href={ROUTES.VENDOR.INDEX}>
                        <Image src={Logo} alt={""} className='w-28' />
                    </Link>
                    <div className="flex flex-col items-center gap-1.5">
                        <p className='text-base font-medium text-black'>Vendor</p>
                    </div>
                </div>
                                    ...
                                    <div className='flex flex-col gap-3'>
                                        {
                                            navSection.navItems?.map((navItem) => (
                                                <Link key={navItem.id} href={navItem.link} className={`flex items-center gap-3 cursor-pointer rounded-md px-4 py-2.5 whitespace-nowrap ${collapse ? "justify-center" : ""} ${pathname.includes(navItem.link) ? "bg-primary text-white font-semibold" : ((pathname==="/dashboard" || pathname==="/teachers") && !!navItem.root) ? "bg-primary text-white font-semibold" : ""}`}>
                                                    <div>
